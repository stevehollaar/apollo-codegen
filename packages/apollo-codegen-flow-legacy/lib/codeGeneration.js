...
function enumerationDeclaration(generator, type) {
  ...
  graphql_2.sortEnumValues(values).forEach((value, i) => {
    if (!value.description || value.description.indexOf("\n") === -1) {
      generator.printOnNewline(
        `  "${value.value}"${i === nValues - 1 ? ";" : " |"}${printing_1.wrap(
          " // ",
...
function interfaceVariablesDeclarationForOperation(
  generator,
...
function typeDeclarationForOperation(
  generator,
  { operationName, operationType, fields }
) {
  const interfaceName = interfaceNameFromOperation({
    operationName,
    operationType
  ...
  language_1.typeDeclaration(
    ...
    () => {
      if (graphql_1.isAbstractType(typeCondition)) {
        ...
        language_1.propertySetsDeclaration(
          generator,
          fragment,
          propertySets,
          true
  ...
  if (graphql_1.isCompositeType(namedType)) {
    ...
    if (fieldType instanceof graphql_1.GraphQLList) {
      ...
      isArrayElementNullable = !(
        fieldType.ofType instanceof graphql_1.GraphQLNonNull
      );
    } else if (
      ...
      isArray = true;
      isArrayElementNullable = !(
        fieldType.ofType.ofType instanceof graphql_1.GraphQLNonNull
      );
    }
...
function propertyDeclarations(generator, properties, isInput) {
  ...
  properties.forEach(property => {
    if (
      graphql_1.isAbstractType(
        graphql_1.getNamedType(property.type || property.fieldType)
      )
    ) {
      ...
      if (
        ...
        language_1.propertyDeclaration(
          generator,
          Object.assign({}, property, { isInput })
        );
      }
    }
  });
...
function getPossibleTypeNames(generator, property) {
  return generator.context.schema
    .getPossibleTypes(
      graphql_1.getNamedType(property.fieldType || property.type)
    )
    .map(type => type.name);
}
