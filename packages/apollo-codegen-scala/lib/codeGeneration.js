    ...
    () => {
      ...
      if (fragmentsReferenced && fragmentsReferenced.length > 0) {
        ...
        fragmentsReferenced.forEach(fragment => {
          generator.print(
            ` + ${naming_1.caseClassNameForFragmentName(
              fragment
            )}.fragmentString`
          );
        });
      ...
      if (variables && variables.length > 0) {
        ...
        language_1.caseClassDeclaration(
          generator,
          {
...
function caseClassDeclarationForSelectionSet(
  generator,
  {
    caseClassName,
    parentType,
  ...
  if (!possibleTypes || possibleTypes.length == 1) {
    const properties = fields
      .map(field =>
        naming_1.propertyFromLegacyField(
          generator.context,
          field,
          caseClassName
        )
      ...
      () => {
        if (inlineFragments && inlineFragments.length > 0) {
          inlineFragments.forEach(inlineFragment => {
            const fragClass = naming_1.caseClassNameForInlineFragment(
              inlineFragment
            );
    ...
    if (inlineFragments && inlineFragments.length > 0) {
      inlineFragments.forEach(inlineFragment => {
        caseClassDeclarationForSelectionSet(generator, {
          caseClassName: naming_1.caseClassNameForInlineFragment(
            inlineFragment
          ),
  ...
  language_1.objectDeclaration(generator, { objectName: caseClassName }, () => {
    ...
    fields
      ...
      .forEach(field => {
        caseClassDeclarationForSelectionSet(generator, {
          caseClassName: naming_1.caseClassNameForPropertyName(
            field.responseName
          ),
