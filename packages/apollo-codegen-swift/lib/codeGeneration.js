            ...
            this.printOnNewline(
              printing_1.wrap(
                ...
                printing_1.join(
                  properties.map(
                    ({ name, propertyName }) =>
                      `"${name}": ${language_1.escapeIdentifierIfNeeded(
                        propertyName
                      )}`
                  ),
                  ", "
        ...
        this.structDeclaration(
          ...
          () => {
            ...
            for (const fragmentSpread of fragmentSpreads) {
              ...
              this.printNewlineIfNeeded();
              this.printOnNewline(
                `public var ${language_1.escapeIdentifierIfNeeded(
                  propertyName
                )}: ${typeName}`
              );
              this.withinBlock(() => {
  ...
  propertyDeclarationForField(field) {
    ...
    this.withinBlock(() => {
      if (graphql_1.isCompositeType(unmodifiedFieldType)) {
        const structName = language_1.escapeIdentifierIfNeeded(
  ...
  initializerDeclarationForProperties(properties) {
    ...
    this.withinBlock(() => {
      properties.forEach(({ propertyName }) => {
        this.printOnNewline(
          `self.${propertyName} = ${language_1.escapeIdentifierIfNeeded(
            propertyName
          )}`
        );
      });
  ...
  enumerationDeclaration(type) {
    ...
    this.withinBlock(() => {
      ...
      values.forEach(value => {
        ...
        this.printOnNewline(
          `case ${language_1.escapeIdentifierIfNeeded(
        ...
        this.withinBlock(() => {
          values.forEach(value => {
            this.printOnNewline(
              `case "${
                value.value
              }": self = ${language_1.escapeIdentifierIfNeeded(
      ...
      this.withinBlock(() => {
        ...
        this.withinBlock(() => {
          values.forEach(value => {
            const enumDotCaseName = language_1.escapeIdentifierIfNeeded(
              this.helpers.enumDotCaseName(value.name)
            );
